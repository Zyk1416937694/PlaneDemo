...
public class Sprite {
    ...
    //目标坐标
    public Rect getDescRect(int status) {
        //目标移动
        move(0, status == STATUS_RUN ? speed * density : 0);
    ...
    public void draw(Canvas canvas, int status) {
        ...
        if (null != bitmap && visibility) {
            canvas.drawBitmap(bitmap, getSrcRect(), getDescRect(status), paint);
        }
        canvasRecf = new Rect(0, 0, canvas.getWidth(), canvas.getHeight());
        //检查是否有交集
        if (!Rect.intersects(canvasRecf, getDescRect(status))) {
            destroy();
        }
    }
